...
export class Lifter extends Vehicle implements IControllable
{
    ...
    private lifter: THREE.Object3D; // Added to change position of the lifter
    ...
    constructor(gltf: any) 
    {
        ...
        this.actions = 
        {
            ...
            'liftUp': new KeyBinding('ArrowUp'),
            'liftDown': new KeyBinding('ArrowDown'),
    ...
    public update(timeStep: number): void {
        ...
        if (this.actions.liftUp.isPressed) {
            this.liftUp();
        }
        if (this.actions.liftDown.isPressed) {
            this.liftDown();
        }
    ...
    public liftDown() {
        let maxLiftDown = -0.15;

        if(this.lifter.position.y > maxLiftDown) {
    ...
    public physicsPreStep(body: CANNON.Body, lifter: Lifter): void
	{
		// Constants
    ...
    public readLifterData(gltf:any): void
    {
        gltf.scene.traverse((child: THREE.Object3D) => {
            if (child.hasOwnProperty('userData'))
            {
                if (child.userData.hasOwnProperty('data'))
                {
                    ...
                    if (child.userData.data === 'lifter')
                    {
                        this.lifter = child;
                        console.log("lifter object set correctly")
                    }
                    ...
                    if (child.userData.liftRight === 'true')
                    {
                        this.lifterColRight = child;
                        console.log("lifter colRight object set correctly")
                    }
                }
            }
        });
